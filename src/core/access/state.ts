...
export interface AccessDencoState extends DencoState {
   ...
   * 
   * ### 無効化の対象
   * 同じ`before_access`で発動する無効化スキル同士は互いに干渉しません. 
   * **すべて無効化スキルが発動して効果が反映されます**
   * 
   * (ex.1)編成にactiveなちとせ・まひるが居る場合
   *  - ちとせ・まひる両方のスキルが発動します
   ...
   * 
   * (ex.2)編成にactiveななる・てすとが居る場合
   * - なるのスコアUP・ATK増加スキルは例外的にスキル無効化と同じ`before_access`で発動する
   * - なる・てすと両方のスキルが発動します
   * - 両編成のcoolでんこ全員のスキルが無効化されます
   * 
   * このとき、coolでんこであるなるのスキルも無効化されますが、
   * **既に発動したスコアUP・ATK増加の効果は消えません**
   * 
   * より実装を詳細に見れば、
   * - `before_access`を含む各段階においてスキル発動の有無を一括で判定
   *   - `skillInvalidated = true`の場合は発動対象外になる
   *   - まだ発動の効果は反映しない
   * - 発動ありと判定されたスキルの発動効果を順次反映する
   * - 次の段階に進む
   */
