...
describe("ひまりのスキル", () => {
  ...
  test.each([1, 2, 3, 4, 5, 6])("発動あり-守備側(編成内)-フィルムx%d", (cnt) => {
    ...
    let defense = initUser(context, "とあるマスター", [seria, mero, miroku, reika, sheena, moe, himari])
    defense.formation.slice(0, cnt).forEach(d => d.film = film1)
    ...
    expect(result.defendPercent).toBe(4.5 * cnt)
  })
