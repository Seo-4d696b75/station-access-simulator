...
describe("しぐれのスキル", () => {
    ...
    test("スキル状態", () => {
        ...
        let sigure = DencoManager.getDenco(context, "21", 1)
        expect(sigure.skill.type).toBe("not_acquired")
        sigure = DencoManager.getDenco(context, "21", 50)
        expect(sigure.skill.type).toBe("possess")
        let state = initUser(context, "とあるマスター", [sigure])
        sigure = state.formation[0]
        let skill = getSkill(sigure)
