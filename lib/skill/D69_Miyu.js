...
var skill = {
    ...
    evaluate: function (context, state, step, self) {
        ...
        var distMax = self.skill.property.readNumber("distMax");
        var expMax = self.skill.property.readNumber("expMax");
        var expDist = Math.floor(expMax * Math.min(1.0, dist / distMax));
        var expFixed = dist >= distMax ? self.skill.property.readNumber("expFixed") : 0;
