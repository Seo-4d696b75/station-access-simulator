...
var skill = {
    onAccessComplete: function (context, state, self, access) {
        ...
        if (target.length > 0) {
            var percent = self.skillPropertyReader("probability");
            var heal_1 = self.skillPropertyReader("heal");
            ...
            return (0, skillEvent_1.evaluateSkillAfterAccess)(context, state, self, access, percent, evaluate);
        }
    },
    disactivateAt: function (context, state, self) {
        var active = self.skillPropertyReader("active");
        var wait = self.skillPropertyReader("wait");
...
//# sourceMappingURL=D01_Seria.js.map