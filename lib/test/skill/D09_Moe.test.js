...
describe("もえのスキル", function () {
    ...
    test("スキル状態", function () {
        ...
        var skill = (0, skill_1.getSkill)(moe);
    ...
    test("スキル発動-1", function () {
        var context = (0, context_1.initContext)("test", "test", false);
        var now = (0, moment_timezone_1.default)("2020-01-01T12:50:00.000").valueOf();
        context.clock = now;
        ...
        expect(state.queue[0].time).toBe((0, moment_timezone_1.default)("2020-01-01T13:00:00.000").valueOf());
        moe = state.formation[0];
        var skill = (0, skill_1.getSkill)(moe);
        expect(skill.state.type).toBe("unable");
        // 13:00
        ...
        expect(state.queue.length).toBe(1);
        expect(state.queue[0].type).toBe("hour_cycle");
        expect(state.queue[0].time).toBe((0, moment_timezone_1.default)("2020-01-01T14:00:00.000").valueOf());
        ...
        if (event.type === "skill_trigger") {
            expect(event.data.time).toBe((0, moment_timezone_1.default)("2020-01-01T14:00:00.000").valueOf());
        ...
        expect(state.queue.length).toBe(1);
        expect(state.queue[0].type).toBe("hour_cycle");
        expect(state.queue[0].time).toBe((0, moment_timezone_1.default)("2020-01-01T15:00:00.000").valueOf());
    });
    test("スキル発動-2", function () {
        ...
        expect(event.type).toBe("skill_trigger");
        if (event.type === "skill_trigger") {
            expect(event.data.time).toBe((0, moment_timezone_1.default)("2020-01-01T13:00:00.000").valueOf());
            expect(event.data.step).toBe("self");
            expect(event.data.denco.name).toBe("moe");
        }
        // check current hp
