...
function isPrimitiveArray(typeName) {
    var func = function (array) {
        return Array.isArray(array) && array.every(function (e) { return typeof e === typeName; });
    };
    return func;
}
function getSkillPropertyReader(property, defaultProperty, typeGuard) {
    return function (key, defaultValue) {
        var value = property.get(key);
        if (value === undefined) {
    ...
    SkillManager.prototype.getSkill = function (numbering, level) {
        ...
        if (data) {
            ...
            if (idx < 0) {
                ...
                var property = data.skillProperties[idx];
                return __assign({ type: "possess", level: property.skillLevel, name: property.name, state: {
                        type: "not_init",
                        transition: data.transition,
                        data: undefined,
                    }, evaluateInPink: data.evaluateInPink, property: {
                        readBoolean: getSkillPropertyReader(property.property, data.skillDefaultProperties, isPrimitive("boolean")),
                        readString: getSkillPropertyReader(property.property, data.skillDefaultProperties, isPrimitive("string")),
                        readNumber: getSkillPropertyReader(property.property, data.skillDefaultProperties, isPrimitive("number")),
                        readStringArray: getSkillPropertyReader(property.property, data.skillDefaultProperties, isPrimitiveArray("string")),
                        readNumberArray: getSkillPropertyReader(property.property, data.skillDefaultProperties, isPrimitiveArray("number"))
                    } }, data.skill);
            }
            else {
