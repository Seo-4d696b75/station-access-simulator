...
function evaluateSkillAfterAccess(context, state, self, access, probability, evaluate) {
    ...
    var eventState = {
        user: (0, user_1.copyUserParam)(state),
        time: access.time,
    ...
    if (result) {
        // スキル発動による影響の反映
        return __assign(__assign({}, result.user), { formation: result.formation.map(function (d) { return (0, denco_1.copyDencoState)(d); }), event: __spreadArray(__spreadArray([], state.event, true), result.event, true), queue: Array.from(state.queue) });
    }
    else {
        return (0, user_1.copyUserState)(state);
    }
...
function randomeAccess(context, state) {
    ...
    var config = {
        offense: {
            state: __assign(__assign({}, state.user), { formation: state.formation.map(function (d) { return (0, denco_1.copyDencoState)(d); }), event: [], queue: [] }),
            carIndex: state.carIndex
        },
        station: station
    };
...
function evaluateSkillAtEvent(context, state, self, probability, evaluate) {
    ...
    if (result) {
        // スキル発動による影響の反映
        next = __assign(__assign({}, result.user), { formation: result.formation.map(function (d) { return (0, denco_1.copyDencoState)(d); }), event: __spreadArray(__spreadArray([], next.event, true), result.event, true), queue: next.queue });
    }
        ...
        switch (entry.type) {
            ...
            case "hour_cycle": {
                ...
                for (var i = 0; i < size; i++) {
                    ...
                    state = callback(context, state, self_1);
                }
                // 次のイベント追加
