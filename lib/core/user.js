...
exports.refreshEXPState = exports.refreshState = exports.copyUserParam = exports.copyUserState = exports.changeFormation = exports.initUser = exports.getTargetDenco = void 0;
...
function copyUserState(state) {
    return __assign(__assign({}, copyUserParam(state)), { formation: Array.from(state.formation).map(function (d) { return (0, denco_1.copyDencoState)(d); }), event: Array.from(state.event), queue: Array.from(state.queue) });
}
...
function refreshState(context, state) {
    ...
    next = (0, skill_1.refreshSkillState)(context, next);
    next = (0, skillEvent_1.refreshEventQueue)(context, next);
...
function refreshEXPState(context, state) {
    ...
    return indices.reduce(function (next, idx) { return refreshEXPStateOne(context, next, idx); }, state);
}
exports.refreshEXPState = refreshEXPState;
function refreshEXPStateOne(context, state, idx) {
    var before = state.formation[idx];
    var after = checkLevelup(context, before);
    if (after) {
        state.formation[idx] = after;
        // 新規にスキル獲得した場合はスキル状態を初期化
        state = (0, skill_1.refreshSkillStateOne)(context, state, idx);
        var event_1 = {
            type: "levelup",
            data: {
