...
function copySkill(skill) {
    if (skill.type === "possess") {
        return __assign(__assign({}, skill), { name: skill.name, level: skill.level, property: skill.property, state: copySkillState(skill.state) });
    }
    return {
        type: skill.type,
    };
...
function refreshSkillState(context, state) {
    ...
    */
    var anyChange = true;
    while (anyChange) {
        anyChange = indices.some(function (idx) { return refreshSkillStateOne(context, state, idx); });
    }
}
    ...
    switch (skill.state.transition) {
        ...
        case "manual-condition": {
            ...
            if (skill.state.type === "idle" || skill.state.type === "unable") {
                ...
                var self_1 = __assign(__assign({}, denco), { carIndex: idx, skill: skill });
        ...
        case "auto-condition": {
            ...
            var self_2 = __assign(__assign({}, denco), { carIndex: idx, skill: skill });
            ...
            if (active && skill.state.type === "unable") {
                ...
                if (callback) {
                    // 更新したスキル状態をコピー
                    self_2 = __assign(__assign({}, (0, denco_1.copyDencoState)(denco)), { carIndex: idx, skill: skill });
